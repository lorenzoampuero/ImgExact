...
- **Decision (2026-09-19): production domain = `imgexact.site`; canonical host = `https://www.imgexact.site`** (the apex 308-redirects to it — aligned same day). DNS screening showed no records on `.site` or `.com` (registrar WHOIS confirms at purchase). The origin is built into `src/config/site.ts` as the default; `PUBLIC_SITE_URL` remains as a staging override only.
