...
| 16 | `/guides` | Informational index | Image guides — formats, compression, sizes and DPI | Image guides | 2026-09-22 | "Explain the decisions behind the tools." | Live |
| 17 | `/guides/image-compression-explained` | Informational: how compression works, why an exact KB is impossible | Image Compression Explained (Lossy, Lossless, Exact KB) | Image compression explained: lossy, lossless, and why no tool can hit an exact KB | 2026-09-22 | "What is actually happening to my file when I set quality 0.8?" | Live |
| 18 | `/guides/best-image-format-for-web` | Informational: which format to choose | WebP vs JPEG vs AVIF vs PNG: Which Format to Use | Which image format to use: WebP vs JPEG vs AVIF vs PNG | 2026-09-22 | "Which format should I send to this place?" | Live |
| 19 | `/guides/free-image-compressor-alternatives` | Commercial investigation: choosing a compressor | Free Image Compressor Alternatives: How to Choose | Free image compressor alternatives: what to check before you upload a file | 2026-09-22 | "Which free compressor can I actually trust with this file?" | Live |
| 20 | `/guides/image-size-vs-dimensions-explained` | Informational: terminology (bytes vs pixels vs DPI) | Image Size vs Dimensions vs Resolution vs DPI | Image size vs dimensions vs resolution vs DPI: what each one means | 2026-09-22 | "The form said 300 DPI and I do not know which number it wants." | Live |
...
| 2026-09-22 | Keyword coverage pass on all 11 tool pages | `keywords`, `taskGuide` and `requirementPresets` added to `src/data/tools.ts` and rendered by `ToolLayout.astro` (new H2 "Common requirements" + a keyword-bearing task section). Titles/metas rewritten where a secondary phrase was missing. Test-enforced by `tests/seo-content.test.ts`: every declared phrase must appear verbatim in rendered text, and no two tools may claim the same phrase. |
| 2026-09-22 | Internal linkification of prose (`src/seo/linkify.ts`) | FAQ answers, how-it-works steps and methodology items now render tool mentions as real links (escaped, max 2 per string, never self-referential). Fixes the unfulfilled promise in the resize tool's "which this page links to". |
| 2026-09-22 | 5 new informational URLs: `/guides` + 4 guides (16–20) | Each row records its intent and its anti-cannibalization rationale (`cannibalization` field in `src/data/guides.ts`, asserted by `tests/guides.test.ts`). No numeric variants, no platform/country pages. Sitemap now 20 URLs; `prod-check` expects 20. |
| 2026-09-22 | Technical + trust fixes | `trailingSlash: false` (duplicate `/slug/` URLs), `og:locale`, `og:image:alt`, `Organization.logo`, real contact channel on `/about` (was future-tense placeholder), `apple-touch-icon` + web manifest. |
| 2026-09-22 | Brand accent: default blue → ink teal + hero sample image | Palette was Tailwind's default (`#2563eb` + slate); now `#0b6e63` with a dark-mode ramp. Guarded by `npm run contrast` (26 WCAG pairs) and a fresh axe-core pass (clean in light and dark on home, tool and guide pages). No URL or intent changes. |
