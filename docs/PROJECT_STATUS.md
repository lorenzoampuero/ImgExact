...
**Live deployment of the keyword/guide/brand pass (2026-09-22, deployed same day):** commit `a83fa0d` pushed to `main` and auto-deployed; `prod-check` against `https://www.imgexact.site` → **23/23 PASS** with zero warnings, including the new checks (served-HTML keyword guards, brand & hero assets, `trailing slash redirect — 308 → /resize-image` from `vercel.json` `trailingSlash: false`). Live CSS bundle `/_astro/BaseLayout.*.css` contains the new accent `#0b6e63` and no longer contains `#2563eb`. IndexNow submission of the five new guide URLs → **202 Accepted**. Operator steps still open: Search Console + Bing verification, sitemap submission and indexing requests (Search Console can only be done by the domain owner).
