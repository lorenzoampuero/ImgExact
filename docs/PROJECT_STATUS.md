...
**Last updated:** 2026-09-20 (web analytics + redesign/dark mode + functionality + SEO/indexing pass) · **Status vocabulary:** DONE (implemented) · TESTED (verified by execution) · PARTIAL · BLOCKED · NOT STARTED
...
| **6 — SEO** | Unique intent/title/meta/H1/canonical, schema, sitemap, robots, internal links | **TESTED** | Registry tests enforce uniqueness + symmetric internal-link graph + anti-doorway slug policy; live checks: canonical `https://www.imgexact.site/...`, 2 JSON-LD blocks/tool page, `robots.txt` 200 + sitemap reference, `sitemap.xml` 200 with 15 `<loc>`. **2026-09-20:** FAQPage schema added on tool pages + homepage from visible Q&A (unit-tested; `prod-check` asserts question/answer visibility), and optional Google/Bing verification meta tags wired to env vars. |
